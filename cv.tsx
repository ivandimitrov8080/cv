    ...
    <Page size="A4" style={tw("w-full h-full text-slate-50 bg-gray-900 flex flex-col p-12 text-base")}>
      <View style={tw("text-center border-2 border-slate-50 rounded-full")}>
        ...
        <View style={tw("flex flex-row gap-4 w-full text-sm justify-center p-4")}>
          {data.github && <SvgLink text="GitHub" href={`https://github.com/${data.github}`} icon={svg.github as any} />}
          {data.upwork && <SvgLink text="Upwork" href={`https://www.upwork.com/freelancers/${data.upwork}`} icon={svg.link as any} />}
          <SvgLink text={data.email} href={`mailto:${data.email}`} icon={svg.email as any} />
          {data.phone && <SvgLink text={data.phone} href={`tel:${data.phone}`} icon={svg.phone as any} />}
          {data.website && <SvgLink text={data.website} href={`https://${data.website}`} icon={svg.globe as any} />}
        </View>
      </View>
